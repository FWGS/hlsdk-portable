...
DECLARE_MESSAGE(m_HealthBar, CheckHPbar)

int CHudHealthBar::Init(void)
{
	m_iOn = 0;
	m_flHealth = 0;

	HOOK_MESSAGE(CheckHPbar);
...
int CHudHealthBar::VidInit(void)
{
	...
	m_hSprite3 = LoadSprite("sprites/fdm_hpicon.spr");
	m_hSprite4 = LoadSprite("sprites/fdm_darkicon.spr");
	return 1;
};


int CHudHealthBar:: MsgFunc_CheckHPbar(const char *pszName,  int iSize, void *pbuf )
{
...
int CHudHealthBar::Draw(float flTime)
{
	...
	if(m_flHealth <= m_iMaxHealth * 0.2)
	{	
		//20%
		UnpackRGB(r,g,b, RGB_REDISH);
	}
	else if(m_flHealth <= m_iMaxHealth * 0.5)
	{
		//50%
