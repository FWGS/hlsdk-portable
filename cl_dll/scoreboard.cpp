...
int CHudScoreboard::Draw( float fTime )
{
	...
	int ypos = ROW_TOP + TopLevelGap() + ( list_slot * ROW_GAP );
	...
	if( cl_scoreboard_bg && cl_scoreboard_bg->value )
		gHUD.DrawDarkRectangle( xpos - 31, ypos - TOP_PADDING, FAR_RIGHT + 26, ScoreboardHeight() );
	if( !gHUD.m_Teamplay )
	...
	list_slot += 1.2f;
	ypos = ROW_TOP + TopLevelGap() + ( list_slot * ROW_GAP );
	...
	while( 1 )
	{
		...
		DrawUtfString( xpos, ypos, NAME_RANGE_MAX + xpos_rel, teamHeader, r, g, b );
