...
void CPipeWrench::Spawn()
{
	...
	SET_MODEL(ENT(pev), "models/w_pipewrench.mdl");
	m_iSwingMode = 0;
	m_iClip = -1;
...
{
	PRECACHE_MODEL("models/v_pipewrench.mdl");
	PRECACHE_MODEL("models/w_pipewrench.mdl");
	PRECACHE_MODEL("models/p_pipewrench.mdl");
	...
	else
	{
		...
		if( pEntity )
		{
...
#endif
			{
				...
				flDamage = gSkillData.plrDmgPipewrench;
			}
			else
			{
				// subsequent swings do half
				flDamage = gSkillData.plrDmgPipewrench / 2;
			}
...
void CPipeWrench::WeaponIdle(void)
{
	...
	{
		m_flNextSecondaryAttack = m_flNextPrimaryAttack = m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 1.0;
		BigSwing();
		m_iSwingMode = 0;
		return;
	}
