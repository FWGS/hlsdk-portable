...
void CAGrunt::PrescheduleThink( void )
{
	if( ShouldSpeak() )
	{
		if( m_flNextWordTime < gpGlobals->time )
		{
			...
			// play a new sound
			if( pev->message )
				pszSound = STRING( szCustomIdleSounds[num] );
			else
				pszSound = pIdleSounds[num];
...
void CAGrunt::DeathSound( void )
{
	...
	const char *pszSound;
	int iRand = RANDOM_LONG( 0, ARRAYSIZE( pDieSounds ) - 1 );

	if( pev->message )
		pszSound = STRING( szCustomDieSounds[iRand] );
	else
...
void CAGrunt::HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case AGRUNT_AE_RIGHT_FOOT:
		...
		{
		case 0:
			EMIT_SOUND_DYN( ENT( pev ), CHAN_BODY, "zork/rightfoot1.wav", 1, ATTN_NORM, 0, 70 );
			break;
		case 1:
			EMIT_SOUND_DYN( ENT( pev ), CHAN_BODY, "zork/rightfoot3.wav", 1, ATTN_NORM, 0, 70 );
			break;
		}
		break;
...
BOOL CAGrunt::CheckRangeAttack1( float flDot, float flDist )
{
	...
	if( !FBitSet( pev->spawnflags, SF_SQUADMONSTER_NOZAP ) && HasConditions( bits_COND_SEE_ENEMY ) && flDist >= AGRUNT_MELEE_DIST && flDist <= 1024.0f && flDot >= 0.5f && NoFriendlyFire() )
	{
		TraceResult tr;
		Vector	vecArmPos, vecArmDir;
