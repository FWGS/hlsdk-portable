...
void CAlienFlyer :: Spawn( void )
{
	...
	m_pBeam[1] = CBeam::BeamCreate( "sprites/xenobeam.spr", 200 );
	...
	m_pBeam[1]->SetColor( 254, 244, 233 );
	m_pBeam[1]->SetNoise( 3 );
	m_pBeam[1]->SetBrightness( 150 );
	m_pBeam[1]->SetWidth( 255 );
	m_pBeam[1]->SetScrollRate( 40 );

	m_pBeam[2] = CBeam::BeamCreate( "sprites/laserbeam.spr", 200 );
	m_pBeam[2]->SetFlags( SF_BEAM_SHADEIN );
	m_pBeam[2]->EntsInit( entindex( ), entindex( ) );
	m_pBeam[2]->SetStartAttachment( 0 );
	m_pBeam[2]->SetEndAttachment( 1 );
	m_pBeam[2]->SetColor( 255, 255, 255 );
	m_pBeam[2]->SetNoise( 100 );
	m_pBeam[2]->SetBrightness( 125 );
	m_pBeam[2]->SetWidth( 30 );
	m_pBeam[2]->SetScrollRate( 35 );
...
void CAlienFlyer::UseDamagingBeam( bool bTurnOn )
{
	if ( bTurnOn )
	{
		...
		m_pBeam[2]->pev->effects &= ~EF_NODRAW;
	} else
	{
		m_pBeam[1]->pev->effects |= EF_NODRAW;
		m_pBeam[2]->pev->effects |= EF_NODRAW;
