...
class CCameraSci : public CTalkMonster
{
public:
	void Spawn(void);
	void Precache(void);
...
LINK_ENTITY_TO_CLASS(monster_camerasci, CCameraSci);

TYPEDESCRIPTION	CCameraSci::m_SaveData[] =
{
	DEFINE_FIELD(CCameraSci, m_painTime, FIELD_TIME),
	DEFINE_FIELD(CCameraSci, m_healTime, FIELD_TIME),
	DEFINE_FIELD(CCameraSci, m_fearTime, FIELD_TIME),
};

IMPLEMENT_SAVERESTORE(CCameraSci, CTalkMonster);


int CCameraSci::FIdleSpeak(void)
{
...
Schedule_t	slCamSciFollow[] =
{
	{
		tlCamSciFollow,
		ARRAYSIZE(tlCamSciFollow),
		bits_COND_NEW_ENEMY |
...
Schedule_t	slCamSciFollowScared[] =
{
	{
		tlCamSciFollowScared,
		ARRAYSIZE(tlCamSciFollowScared),
		bits_COND_NEW_ENEMY |
		bits_COND_HEAR_SOUND |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE,
...
Task_t	tlCamSciFaceTargetScared[] =
{
...
Schedule_t	slCamSciFaceTargetScared[] =
{
	{
		tlCamSciFaceTargetScared,
		ARRAYSIZE(tlCamSciFaceTargetScared),
		bits_COND_HEAR_SOUND |
		bits_COND_NEW_ENEMY,
		bits_SOUND_DANGER,
		"FaceTargetScared"
...
Task_t	tlCamSciStopFollowing[] =
{
	{ TASK_CANT_FOLLOW, (float)0 },
};

Schedule_t	slCamSciStopFollowing[] =
{
	{
		tlCamSciStopFollowing,
		ARRAYSIZE(tlCamSciStopFollowing),
		0,
		0,
		"StopFollowing"
	},
...
Schedule_t	slCamSciHeal[] =
{
	{
		tlCamSciHeal,
		ARRAYSIZE(tlCamSciHeal),
...
Schedule_t	slCamSciFaceTarget[] =
{
	{
		tlCamSciFaceTarget,
		ARRAYSIZE(tlCamSciFaceTarget),
...
Schedule_t	slCamSciSciPanic[] =
{
	{
		tlCamSciSciPanic,
		ARRAYSIZE(tlCamSciSciPanic),
		0,
		0,
		"SciPanic"
	},
...
Schedule_t	slCamSciIdleSciStand[] =
{
	{
		tlCamSciIdleSciStand,
		ARRAYSIZE(tlCamSciIdleSciStand),
		bits_COND_NEW_ENEMY |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE |
		bits_COND_HEAR_SOUND |
...
Schedule_t	slCamSciScientistCover[] =
{
	{
		tlCamSciScientistCover,
		ARRAYSIZE(tlCamSciScientistCover),
		bits_COND_NEW_ENEMY,
		0,
		"ScientistCover"
	},
...
Schedule_t	slCamSciScientistHide[] =
{
	{
		tlCamSciScientistHide,
		ARRAYSIZE(tlCamSciScientistHide),
		bits_COND_NEW_ENEMY |
		bits_COND_HEAR_SOUND |
		bits_COND_SEE_ENEMY |
		bits_COND_SEE_HATE |
...
Task_t	tlCamSciScientistStartle[] =
{
	{ TASK_SET_FAIL_SCHEDULE, (float)SCHED_PANIC },		// If you fail, just panic!
...
Schedule_t	slCamSciScientistStartle[] =
{
	{
		tlCamSciScientistStartle,
		ARRAYSIZE(tlCamSciScientistStartle),
		bits_COND_NEW_ENEMY |
		bits_COND_SEE_ENEMY |
		bits_COND_SEE_HATE |
		bits_COND_SEE_FEAR |
...
Schedule_t	slCamSciFear[] =
{
	{
		tlCamSciFear,
		ARRAYSIZE(tlCamSciFear),
		bits_COND_NEW_ENEMY,
		0,
		"Fear"
	},
...
DEFINE_CUSTOM_SCHEDULES(CCameraSci)
{
	slCamSciFollow,
		slCamSciFaceTarget,
		slCamSciIdleSciStand,
		slCamSciFear,
		slCamSciScientistCover,
		slCamSciScientistHide,
		slCamSciScientistStartle,
		slCamSciHeal,
		slCamSciStopFollowing,
		slCamSciSciPanic,
		slCamSciFollowScared,
		slCamSciFaceTargetScared,
};


IMPLEMENT_CUSTOM_SCHEDULES(CCameraSci, CTalkMonster);


void CCameraSci::DeclineFollowing(void)
{
	if (BeingShocked)
		return;
...
Activity CCameraSci::GetStoppedActivity(void)
{
	if (m_hEnemy != NULL)
		return ACT_EXCITED;
...
void CCameraSci::StartTask(Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_SAY_HEAL:
...
void CCameraSci::RunTask(Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_RUN_PATH_SCARED:
...
//=========================================================
void CCameraSci::SetYawSpeed(void)
{
	int ys;
...
//=========================================================
void CCameraSci::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	switch (pEvent->event)
	{
...
void CCameraSci::CommitSudoku(void)
{
...
// Spawn
//=========================================================
void CCameraSci::Spawn(void)
{
	Precache();

	SET_MODEL(ENT(pev), "models/camerasci.mdl");
...
//=========================================================
void CCameraSci::Precache(void)
{
	PRECACHE_MODEL("models/camerasci.mdl");
	PRECACHE_SOUND("scientist/sci_pain1.wav");
	PRECACHE_SOUND("scientist/sci_pain2.wav");
	PRECACHE_SOUND("scientist/sci_pain3.wav");
	PRECACHE_SOUND("scientist/sci_pain4.wav");
	PRECACHE_SOUND("scientist/sci_pain5.wav");
	PRECACHE_SOUND("weapons/camerasnap.wav");
...
void CCameraSci::TalkInit()
{
	...
	case HEAD_LUKA:		m_voicePitch = 90;  break;  //Lukashenko
	case HEAD_VORT:		m_voicePitch = 70;  break;  //Vortigaunt
	}
}

int CCameraSci::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
...
//=========================================================
int CCameraSci::ISoundMask(void)
{
...
// PainSound
//=========================================================
void CCameraSci::PainSound(void)
{
...
// DeathSound 
//=========================================================
void CCameraSci::DeathSound(void)
{
	PainSound();
}

void CCameraSci::Killed(entvars_t *pevAttacker, int iGib)
{
	SetUse(NULL);
	CTalkMonster::Killed(pevAttacker, iGib);
	if (FClassnameIs(pevAttacker, "monster_madscientist"))
	{
...
void CCameraSci::SetActivity(Activity newActivity)
{
	int	iSequence;
...
Schedule_t* CCameraSci::GetScheduleOfType(int Type)
{
	Schedule_t *psched;
	...
	case SCHED_TARGET_FACE:
		...
		if (psched == slIdleStand)
			return slCamSciFaceTarget;	// override this for different target face behavior
		else
			return psched;

	case SCHED_TARGET_CHASE:
		return slCamSciFollow;

	case SCHED_CANT_FOLLOW:
		return slCamSciStopFollowing;

	case SCHED_PANIC:
		return slCamSciSciPanic;

	case SCHED_TARGET_CHASE_SCARED:
		return slCamSciFollowScared;

	case SCHED_TARGET_FACE_SCARED:
		return slCamSciFaceTargetScared;
	...
	case SCHED_IDLE_STAND:
		...
		if (psched == slIdleStand)
			return slCamSciIdleSciStand;
		else
			return psched;

	case SCHED_HIDE:
		return slCamSciScientistHide;

	case SCHED_STARTLE:
		return slCamSciScientistStartle;
...
Schedule_t *CCameraSci::GetSchedule(void)
{
...
MONSTERSTATE CCameraSci::GetIdealState(void)
{
	switch (m_MonsterState)
	{
	case MONSTERSTATE_ALERT:
...
void CCameraSci::Heal(void)
{
	...
	if (m_hEnemy)
		m_hEnemy->TakeDamage(pev, pev, 1, DMG_BURN);

	EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "weapons/camerasnap.wav", 1.0, ATTN_NORM, 0, 100);

	if (LastShitTalked < gpGlobals->time)
	{
		LastShitTalked = gpGlobals->time + 5;
		SENTENCEG_PlayRndSz(ENT(pev), "SC_CRINGE", 1.0, ATTN_IDLE, 0, GetVoicePitch());
	}

	Vector vecSrc;
	vecSrc = pev->origin + gpGlobals->v_up * 36 + gpGlobals->v_forward * 12;
...
int CCameraSci::FriendNumber(int arrayNumber)
{
...
BOOL CCameraSci::CheckMeleeAttack1(float flDot, float flDist)
{
	if (flDot >= 0.7 && flDist <= 150)
	{
		return TRUE;
	}
	return FALSE;
