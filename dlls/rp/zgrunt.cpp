...
const char *CZGrunt::pGruntSentences[] =
{
	...
	"ZG_ALERT", // sees player
	"ZG_MONSTER", // sees monster
	"ZG_COVER", // running to cover
	"ZG_THROW", // about to throw grenade
	"ZG_CHARGE",  // running out to get the enemy
	"ZG_TAUNT", // say rude things
};
...
// started moving.
//=========================================================
void CZGrunt::SpeakSentence( void )
{
...
//=========================================================
int CZGrunt::IRelationship( CBaseEntity *pTarget )
{
	if( FClassnameIs( pTarget->pev, "monster_human_grunt" ) || ( FClassnameIs( pTarget->pev,  "monster_male_assassin" ) ) )
	{
		return R_NM;
	}
...
void CZGrunt::IdleSound( void )
{
	if( FOkToSpeak() && ( g_fGruntQuestion || RANDOM_LONG( 0, 1 ) ) )
	{
		if( !g_fGruntQuestion )
		{
			...
			case 0:
				// check in
				SENTENCEG_PlayRndSz( ENT( pev ), "ZG_CHECK", HGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch );
				g_fGruntQuestion = 1;
				break;
			case 1:
				// question
				SENTENCEG_PlayRndSz( ENT( pev ), "ZG_QUEST", HGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch );
				g_fGruntQuestion = 2;
				break;
			case 2:
				// statement
				SENTENCEG_PlayRndSz( ENT( pev ), "ZG_IDLE", HGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch );
				break;
			}
		}
		else
		{
			...
			case 1:
				// check in
				SENTENCEG_PlayRndSz( ENT( pev ), "ZG_CLEAR", HGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch );
				break;
			case 2:
				// question
				SENTENCEG_PlayRndSz( ENT( pev ), "ZG_ANSWER", HGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch );
				break;
			}
			g_fGruntQuestion = 0;
		}
	...
	switch( pEvent->event )
	{
		...
		case HGRUNT_AE_KICK:
		{
			...
			if( pHurt )
			{
				...
				pHurt->TakeDamage( pev, pev, gSkillData.zgruntDmgKick, DMG_CLUB );
			}
		}
			break;
...
void CZGrunt::PainSound( void )
{
	if( gpGlobals->time > m_flNextPainTime )
	{
		...
		if( RANDOM_LONG( 0, 99 ) < 5 )
		{
			...
			if( FOkToSpeak() )
			{
				SENTENCEG_PlayRndSz( ENT( pev ), "ZG_PAIN", HGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, PITCH_NORM );
				JustSpoke();
				return;
			}
		}
...
// DeathSound 
//=========================================================
void CZGrunt::DeathSound( void )
{
	switch( RANDOM_LONG( 0, 2 ) )
	{
	case 0:
		EMIT_SOUND( ENT( pev ), CHAN_VOICE, "agrunt/ag_die1.wav", 1, ATTN_IDLE );
		break;
	case 1:
		EMIT_SOUND( ENT( pev ), CHAN_VOICE, "agrunt/ag_die2.wav", 1, ATTN_IDLE );
		break;
	case 2:
		EMIT_SOUND( ENT( pev ), CHAN_VOICE, "agrunt/ag_die3.wav", 1, ATTN_IDLE );
		break;
	case 3:
		EMIT_SOUND( ENT( pev ), CHAN_VOICE, "agrunt/ag_die4.wav", 1, ATTN_IDLE );
		break;
	case 4:
		EMIT_SOUND( ENT( pev ), CHAN_VOICE, "agrunt/ag_die5.wav", 1, ATTN_IDLE );
		break;
	}
}
...
// Get Schedule!
//=========================================================
Schedule_t *CZGrunt::GetSchedule( void )
{
	...
	if( HasConditions( bits_COND_HEAR_SOUND ) )
	{
		...
		if( pSound )
		{
			if( pSound->m_iType & bits_SOUND_DANGER )
			{
				...
				if( FOkToSpeak() )
				{
					SENTENCEG_PlayRndSz( ENT( pev ), "ZG_GREN", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch );
					JustSpoke();
				}
	...
	case MONSTERSTATE_COMBAT:
		{
					...
					else 
					{
						...
						if( FOkToSpeak() )// && RANDOM_LONG( 0, 1 ) )
						{
							if( ( m_hEnemy != 0 ) && m_hEnemy->IsPlayer() )
								...
								// monster
								SENTENCEG_PlayRndSz( ENT( pev ), "ZG_MONST", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch );
			...
			else if( HasConditions( bits_COND_LIGHT_DAMAGE ) )
			{
				...
				if( iPercent <= 90 && m_hEnemy != 0 )
				{
					...
					if( FOkToSpeak() ) // && RANDOM_LONG( 0, 1 ) )
					{
						//SENTENCEG_PlayRndSz( ENT( pev ), "ZG_COVER", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch );
			...
			else if( HasConditions( bits_COND_ENEMY_OCCLUDED ) )
			{
				if( HasConditions( bits_COND_CAN_RANGE_ATTACK2 ) && OccupySlot( bits_SLOTS_HGRUNT_GRENADE ) )
				{
					...
					if( FOkToSpeak() )
					{
						SENTENCEG_PlayRndSz( ENT( pev ), "ZG_THROW", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch );
						JustSpoke();
					}
					return GetScheduleOfType( SCHED_RANGE_ATTACK2 );
				}
				else if( OccupySlot( bits_SLOTS_HGRUNT_ENGAGE ) )
				{
					...
					if( FOkToSpeak() )// && RANDOM_LONG( 0, 1 ) )
					{
						//SENTENCEG_PlayRndSz( ENT( pev ), "ZG_CHARGE", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch );
				...
				else
				{
					...
					if( FOkToSpeak() && RANDOM_LONG( 0, 1 ) )
					{
						SENTENCEG_PlayRndSz( ENT( pev ), "ZG_TAUNT", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch );
						JustSpoke();
					}
...
//=========================================================
//=========================================================
Schedule_t *CZGrunt::GetScheduleOfType( int Type ) 
{
	switch( Type )
	{
...
void CZGruntRepel::Spawn( void )
{
	Precache();
	pev->solid = SOLID_NOT;

	SetUse( &CZGruntRepel::RepelUse );
}

void CZGruntRepel::Precache( void )
{
	UTIL_PrecacheOther( "monster_zombie_grunt" );

	m_iSpriteTexture = PRECACHE_MODEL( "sprites/rope.spr" );
}

void CZGruntRepel::RepelUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	TraceResult tr;
	...
	*/
	...
	CBaseEntity *pEntity = Create( "monster_zombie_grunt", pev->origin, pev->angles );
...
const char *CDeadZGrunt::m_szPoses[] = { "deadstomach", "deadside", "deadsitting" };
...
void CDeadZGrunt::Spawn( void )
{
	...
	if( pev->sequence == -1 )
	{
		ALERT( at_console, "Dead zgrunt with bad pose\n" );
	}
