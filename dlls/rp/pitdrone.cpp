...
void CPitdrone::Spawn()
{
	...
	m_flFieldOfView = -0.7ccсс;// indicates the width of this monster's forward view cone ( as a dotproduct result )
...
#define	PITDRONE_SPRINT_DIST			256
#define PITDRONE_FLINCH_DELAY			20		// at most one flinch every n secs
...
void CPitDrone::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	case PIT_DRONE_AE_SLASH:
	{
		 ...
		 */
		// SOUND HERE (in the pitdrone model)
		CBaseEntity *pHurt = CheckTraceHullAttack(70, gSkillData.pitdroneDmgBite, DMG_CLUB | DMG_ALWAYSGIB );
		if (pHurt)
		{
			if (pHurt->pev->flags & (FL_MONSTER | FL_CLIENT))
			{
				pHurt->pev->punchangle.z = shouldAttackWithLeftClaw ? 20 : -20;
				pHurt->pev->punchangle.x = 20;
				pHurt->pev->velocity = pHurt->pev->velocity + gpGlobals->v_right * ( shouldAttackWithLeftClaw ? 200 : -200 );
				pHurt->pev->velocity = pHurt->pev->velocity + gpGlobals->v_up * 100;
			}
			EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, pAttackMissSounds[RANDOM_LONG(0, ARRAYSIZE(pAttackHitSounds) - 1)], 1.0, ATTN_NORM, 0, 100 + RANDOM_LONG(-5, 5));
		}
...
void CPitDrone::Spawn()
{
	...
	pev->effects = 0;
	pev->health = gSkillData.pitdroneHealth;
	m_flFieldOfView = -0.7;// indicates the width of this monster's forward view cone ( as a dotproduct result )
	m_MonsterState = MONSTERSTATE_NONE;
