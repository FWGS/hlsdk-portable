...
//=========================================================
// Friendly sentry - a player built sentry (MAKIN BACON)
//=========================================================
class CBuiltSentry : public CBaseTurret
{
public:
	void Spawn();
	void Precache(void);
...
LINK_ENTITY_TO_CLASS(monster_builtsentry, CBuiltSentry);

void CBuiltSentry::Precache()
{
	CBaseTurret::Precache();
	PRECACHE_MODEL("models/sentry.mdl");
}

void CBuiltSentry::Spawn()
{
	Precache();
	SET_MODEL(ENT(pev), "models/sentry.mdl");
	pev->health = gSkillData.sentryHealth;
	...
	pitchbyhealth = (100 - pev->health);
	m_iRetractHeight = 64;
	m_iDeployHeight = 64;
	m_iMinPitch = -60;
	UTIL_SetSize(pev, Vector(1, 1, 1), Vector(16, 16, m_iRetractHeight));


	UTIL_SetOrigin(pev, pev->origin);

	SetTouch(&CBuiltSentry::SentryTouch);
	SetThink(&CBaseTurret::Initialize);

	pev->nextthink = gpGlobals->time + 0.3;
}

void CBuiltSentry::Shoot(Vector &vecSrc, Vector &vecDirToEnemy)
{
	FireBullets(1, vecSrc, vecDirToEnemy, TURRET_SPREAD, TURRET_RANGE, BULLET_MONSTER_MP5, 1);
...
int CBuiltSentry::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
	if (!pev->takedamage)
		return 0;
...
void CBuiltSentry::SentryTouch(CBaseEntity *pOther)
{
	if (pOther && (pOther->IsPlayer() || (pOther->pev->flags & FL_MONSTER)))
	{
		TakeDamage(pOther->pev, pOther->pev, 0, 0);
...
void CBuiltSentry::SentryDeath(void)
{
	BOOL iActive = FALSE;
...
int	CBuiltSentry::Classify(void)
{
