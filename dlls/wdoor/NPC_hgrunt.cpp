...
#define		HGRUNT_AE_BURST4		( 14 ) 
#define		HGRUNT_AE_BURST5		( 15 ) 
...
LINK_ENTITY_TO_CLASS( monster_human_grunt_medic, CHGrunt );
LINK_ENTITY_TO_CLASS( monster_human_grunt_torch, CHGrunt );
LINK_ENTITY_TO_CLASS( monster_human_grunt_shape, CHGrunt );
...
//  DEFINE_FIELD( CShotgun, m_iShotgunShell, FIELD_INTEGER ),
	...
	DEFINE_FIELD( CHGrunt, m_iDisplacer, FIELD_INTEGER ),
	DEFINE_FIELD( CHGrunt, m_checkAttackTime, FIELD_TIME ),
	DEFINE_FIELD( CHGrunt, m_lastAttackCheck, FIELD_BOOLEAN ),
};
...
void CHGrunt :: RunAI( void )
{
	CBaseMonster :: RunAI();
		...
		if ( FClassnameIs(pev, "monster_human_grunt_shape")){
		...
		if(m_makerspawn_call == 1){
			if (FBitSet( pev->weapons, HGRUNT_SG550 ))
			{
				SetBodygroup( GUN_GROUP, GUN_SG550 );
				m_cClipSize		= 20;
				m_cAmmoLoaded = m_cClipSize;
			}
		...
		if(m_iDisplacer == 2 && pev->origin.z > 4096 && m_freezetime == 0){//��л��Xen����
		m_iDisplacer = 3;
		m_cAmmoLoaded = m_cClipSize;
...
void CHGrunt::Killed( entvars_t *pevAttacker, int iGib )
{
	...
	if ( pev->weapons >= 1 && GetBodygroup( GUN_GROUP ) != GUN_NONE && g_gibexp_max < 120 && !m_undropgun)
	{// throw a gun if the grunt has one
		...
		CBaseEntity *pGun;
			...
			else if ( FClassnameIs(pev, "monster_human_grunt_medic")){
				pGun = DropItem( "weapon_glock", vecGunPos, vecGunAngles );
			}
			else if (FBitSet( pev->weapons, HGRUNT_SG550 ))
			{
				pGun = DropItem( "weapon_sg550_drop", vecGunPos, vecGunAngles );
			}
			else if (FBitSet( pev->weapons, HGRUNT_SHOTGUN ))
			{
				pGun = DropItem( "weapon_shotgun_limip", vecGunPos, vecGunAngles );
			}
			else
			{
			...
			if ( FClassnameIs(pev, "monster_human_grunt_shape")){//��л�η��װ��!
				vecGunPos.x = pev->origin.x;
				vecGunPos.y = pev->origin.y;

				m_undropgun = TRUE;
				SetBodygroup( GUN_GROUP, GUN_SG550 );

				CBaseEntity *pGrenade;
				pGrenade = DropItem( "weapon_medkit", vecGunPos, vecGunAngles );
				if ( pGrenade )
				{
					g_gibexp_max += 1;
				...
				pGrenade = DropItem( "ammo_ARgrenades", vecGunPos, vecGunAngles );
				if ( pGrenade )
				{
					g_gibexp_max += 1;
				...
				pGrenade = DropItem( "weapon_displacer", vecGunPos, vecGunAngles );
				if ( pGrenade )
				{
					g_gibexp_max += 1;
			...
			{
				CBaseEntity *pGrenade;
				pGrenade = DropItem( "weapon_handgrenade_drop", vecGunPos, vecGunAngles );
				if ( pGrenade )
				{
					g_gibexp_max += 1;
...
BOOL CHGrunt :: CheckMeleeAttack1 ( float flDot, float flDist )
{
	...
	if ( FClassnameIs(pev, "monster_human_grunt_torch")){
		if ( flDist <= 256 && flDot >= 0.5){
			return TRUE;//�����Ա�!
		}
	}
	else{
		if ( flDist <= 70 && flDot >= 0.6	&& 
...
BOOL CHGrunt :: CheckRangeAttack1 ( float flDot, float flDist )
{
	...
	if(m_hEnemy != NULL){
			if ( fabs( pev->origin.z - m_hEnemy->pev->origin.z ) >= 192 ){//����˸߶Ȳ�ϸ�ʱ�����ӹ�������
			dist += 256;
			}
	}
...
void CHGrunt :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	if (ptr->iHitgroup == 8)
	{
			if ( FClassnameIs(pev, "monster_human_grunt_torch") && pev->dmg < 100){//�Ա���
				pev->health = 1;
				FX_Trail(Center(), entindex(), (UTIL_PointContents(pev->origin) == CONTENT_WATER)?PROJ_M203_DETONATE_WATER:PROJ_M203_DETONATE );
				::RadiusDamage2(Center(), pev, pev, 100, 250, CLASS_NONE, DMG_BLAST);
...
{
	if ( FClassnameIs(pev, "monster_human_grunt_shape") && m_walkaround == FALSE){
	m_cleardally_enemy_long = 60;//��͵Ϯ!
	ClearSchedule();
	m_walkaround = TRUE;
...
void CHGrunt :: ShootSniper ( void )
{
	...
	FireBullets(1, vecShootOrigin, vecShootDir, Vector(0.025,0.025,0.025), 2560, iBulletType,1,623); // 2��������
...
void CHGrunt :: Shoot ( void )
{
	...
	if ( FClassnameIs(pev, "monster_human_grunt_shape")){//��׼�Ⱥ�����������
	FireBullets(1, vecShootOrigin, vecShootDir, Vector(0.05,0.05,0.05), 2560, BULLET_MONSTER_12MM,1);
	}
	else{
	FireBullets(1, vecShootOrigin, vecShootDir, Vector(0.075,0.075,0.075), 2048, iBulletType,1);
	}// shoot +-5 degrees
...
void CHGrunt :: Shoot_Pistol ( void )
{
	if ( m_cAmmoLoaded <= 0 )
	return;
	...
	FireBullets(1, vecShootOrigin, vecShootDir, Vector(0.025,0.025,0.025), 2048, iBulletType,1); // shoot +-5 degrees
...
void CHGrunt :: Shotgun ( void )
{
	...
	FireBullets(5, vecShootOrigin, vecShootDir, Vector(0.12,0.12,0.12), 2048, iBulletType,1); // shoot +-7.5 degrees
...
void CHGrunt :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		...
		case HGRUNT_AE_GREN_LAUNCH:
		{
			...
			if ( FClassnameIs(pev, "monster_human_grunt_shape")){
			m_flNextGrenadeCheck = gpGlobals->time + RANDOM_FLOAT( 2, 4 );
			}
			else{
			m_flNextGrenadeCheck = gpGlobals->time + 5;
			}
		...
		case HGRUNT_AE_BURST1:
		{
			...
			{
				ShootSniper();
				EMIT_SOUND( ENT(pev), CHAN_WEAPON, "weapons/sg550-1.wav", 1, ATTN_NORM );
			}
			else if ( FBitSet( pev->weapons, HGRUNT_9MMAR ))
			{
				...
				if ( FClassnameIs(pev, "monster_human_grunt_shape")){
					if ( RANDOM_LONG(0,1) )
					{
						EMIT_SOUND( ENT(pev), CHAN_WEAPON, "weapons/hks1.wav", 1, 0.7 );
					}
					else
					{
						EMIT_SOUND( ENT(pev), CHAN_WEAPON, "weapons/hks2.wav", 1, 0.7 );
					}
				}
				else{
		...
		case HGRUNT_AE_KICK:
		{
			...
			ClearMultiDamage( );
			pEntity->TraceAttack(pev, kick_dmg, gpGlobals->v_forward, &tr, DMG_SLASH ); 
			...
			EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, "newadd/fist_hitbod2.wav", 1.0, ATTN_NORM, 0, 100 + RANDOM_LONG(-5,5) );
				...
				CBaseEntity *pHurt = CheckTraceHullAttack( 80, kick_dmg, DMG_SLASH );
				if ( pHurt )
				{
		...
		case 17:
		{
			EMIT_SOUND( ENT(pev), CHAN_WEAPON, "weapons/displacer_spin.wav", 1, 0.6 );
		}
		break;
		...
		case 18:
		{
			...
			CBaseEntity *pSatchel = Create( "displacer_ball", vecSrc, Vector( 0, 0, 0), edict() );
			pSatchel->pev->velocity = vecThrow;
			pSatchel->pev->owner = edict();

			Freeze_Monster(150);//��Ѫ15�룬60%
			TakeHealth( pev->max_health * 0.6, DMG_GENERIC );
...
void CHGrunt :: Spawn()
{
	...
	if ( oldbody < 0 )
	{
		SetBodygroup( HEAD_GROUP, RANDOM_LONG(0,5));
	}
	else{
		SetBodygroup( HEAD_GROUP, oldbody);
	}
	...
	if (pev->weapons <= 0)
	{
		...
		pev->weapons = HGRUNT_SG550 | HGRUNT_HANDGRENADE;
		}
	...
	if ( GetBodygroup( HEAD_GROUP ) == 2 || GetBodygroup( HEAD_GROUP ) == 3){
	pev->skin = 0;	// light skin
	}
	if ( GetBodygroup( HEAD_GROUP ) == 4 || GetBodygroup( HEAD_GROUP ) == 5){
	...
	m_killed_exp = 80;
	m_rpgms_level = 50;
	pev->netname = MAKE_STRING( "HECU" );

	if ( FClassnameIs(pev, "monster_human_grunt_medic")){
	...
	m_cAmmoLoaded		= m_cClipSize;
	pev->weapons = 1;
	pev->netname = MAKE_STRING( "HECU.Medic" );
	}
	else if ( FClassnameIs(pev, "monster_human_grunt_torch")){
	...
	m_cClipSize	= 1;
	m_cAmmoLoaded		= m_cClipSize;
	m_undropgun = TRUE;//û�ж����ɵ���!
	pev->weapons = 1;
	m_ignoredamage = 1;
	pev->netname = MAKE_STRING( "HECU.Torch" );
	}

	if ( FClassnameIs(pev, "monster_human_grunt_shape")){
		SET_MODEL(ENT(pev), "models/hgrunt_shape.mdl");
		UTIL_SetSize(pev, VEC_HUMAN_HULL_MIN, VEC_HUMAN_HULL_MAX);
		pev->skin = 0;
		pev->body = 0;
		m_ignoredamage = 1;
		...
		if (g_iSkillLevel == SKILL_HARD){//BOSS��
		pev->health			= 3000;
		}
		else{
		pev->health			= 2400;
		...
		pev->weapons = 5;
		pev->netname = MAKE_STRING( "HECU.Shape" );
		m_fGunDrawn = FALSE;
		m_iDisplacer = 1;
...
void CHGrunt :: Precache()
{
	...
	PRECACHE_MODEL("models/hgrunt_shape.mdl");
...
Task_t tlGruntEstablishLineOfFire_strap[] = 
{
	{ TASK_SET_FAIL_SCHEDULE,	(float)SCHED_GRUNT_ELOF_FAIL	},
	{ TASK_GET_PATH_TO_ENEMY,	(float)0						},
...
Schedule_t slGruntEstablishLineOfFire_strap[] =
{
	{ 
		tlGruntEstablishLineOfFire_strap,
		ARRAYSIZE ( tlGruntEstablishLineOfFire_strap ),
		bits_COND_NEW_ENEMY			|
		bits_COND_ENEMY_DEAD		|
		bits_COND_CAN_RANGE_ATTACK1	|
		bits_COND_CAN_MELEE_ATTACK1	|
...
Task_t	tlGruntTakeCoverFromBestSound2[] =
{
	{ TASK_SET_FAIL_SCHEDULE,			(float)SCHED_CHASE_ENEMY			},// duck and cover if cannot move from explosion
	{ TASK_STOP_MOVING,					(float)0					},
	{ TASK_FIND_COVER_FROM_BEST_SOUND,	(float)0					},
	{ TASK_RUN_PATH,					(float)0					},
	{ TASK_WAIT_FOR_MOVEMENT,			(float)0					},
...
Schedule_t	slGruntTakeCoverFromBestSound2[] =
{
	{ 
		tlGruntTakeCoverFromBestSound2,
		ARRAYSIZE ( tlGruntTakeCoverFromBestSound2 ), 
		0,
		0,
...
DEFINE_CUSTOM_SCHEDULES( CHGrunt )
{
	...
	slGruntTakeCoverFromBestSound2,
	slGruntHideReload,
	slGruntSweep,
	...
	slGruntRepelAttack,
	slGruntRepelLand,
	slGruntEstablishLineOfFire_strap,
};
...
void CHGrunt :: SetActivity ( Activity NewActivity )
{
	...
	case ACT_RANGE_ATTACK1:
		if (FBitSet( pev->weapons, HGRUNT_SG550))
		{
			iSequence = LookupSequence( "standing_sg550" );
		}
		else if (FBitSet( pev->weapons, HGRUNT_9MMAR))
		{
	...
	case ACT_RANGE_ATTACK2:
		...
		{
			// get toss anim
			iSequence = LookupSequence( "fire_displacer" );
		}
		else if ( pev->weapons & HGRUNT_HANDGRENADE )
		{
			// get toss anim
...
// dead enemy
			...
			if (FClassnameIs(pev, "monster_human_grunt_shape")){
				if(m_iDisplacer == 1){//һ��������
					if (!m_fGunDrawn && pev->health <= pev->max_health * 0.5){//����
				...
				if (HasConditions ( bits_COND_NO_AMMO_LOADED )){//ԭ�ؿ��ٻ���
				return GetScheduleOfType ( SCHED_RELOAD_DEEP );
				}
			}
...
// damaged just a little
			else if ( HasConditions( bits_COND_LIGHT_DAMAGE ) )
			{
					...
					if ( iPercent <= 90 && pev->health <= pev->max_health * 0.4 )
					//���ˣ�90%��������
					{
...
void CHGruntRepel::Spawn( void )
{
	...
	SetThink( &CHGruntRepel::glock_think );

	SetUse( &CHGruntRepel::RepelUse );
}

void CHGruntRepel :: glock_think( void )
{
	if(pev->frags >= 1){
	TraceResult tr;
	...
	*/
	...
	if(pev->frags == 1 || pev->frags == 2){
	pEntity = Create( "monster_human_grunt", pev->origin, pev->angles );
	...
	pGrunt->m_cClipSize		= 36;
		if(pev->frags == 2){
		pGrunt->m_fightmode = 2;
		}
	}
	else if(pev->frags == 3){//Shape Repel!
	pEntity = Create( "monster_human_grunt_shape", pev->origin, pev->angles );
	pGrunt = pEntity->MyMonsterPointer( );
	pGrunt->pev->movetype = MOVETYPE_FLY;
	pGrunt->pev->velocity = Vector( 0, 0, RANDOM_FLOAT( -196, -128 ) );
	pGrunt->SetActivity( ACT_GLIDE );
	...
	pGrunt->m_alert = 100;
	pGrunt->m_walkaround = TRUE;
	pGrunt->m_walkaroundFail = TRUE;
