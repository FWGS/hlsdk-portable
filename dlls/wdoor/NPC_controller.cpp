...
LINK_ENTITY_TO_CLASS( monster_alien_controller_big, CController );

TYPEDESCRIPTION	CController::m_SaveData[] = 
{
	DEFINE_FIELD( CController, m_flNextSkillCheck, FIELD_TIME ),
...
//=========================================================
BOOL CController :: FCanCheckAttacks ( void )
{
	...
	switch( pEvent->event )
	{
		...
		case CONTROLLER_AE_BALL_SHOOT:
		{
			...
			if(m_hEnemy != NULL){
				...
				if ( FClassnameIs(pev, "monster_alien_controller_big")){
				pBall->pev->scale = 2.0;
				pBall->pev->rendercolor.y = 32;
				pBall->pev->rendercolor.z = 32;
				pBall->pev->dmg = 2;
				}
...
void CController :: Spawn()
{
	...
	m_killed_exp = 80;
	m_rpgms_level = 50;
	pev->netname = MAKE_STRING( "Alien.Controller" );

	if ( FClassnameIs(pev, "monster_alien_controller_big")){
		SET_MODEL(ENT(pev), "models/controller_giant.mdl");
		...
		m_killed_exp = 160;
		m_rpgms_level = 60;
		pev->netname = MAKE_STRING( "Hell.Controller" );
		SetEyePosition();
	}
}
...
void CController :: Precache()
{
	...
	PRECACHE_MODEL("models/controller_giant.mdl");
//	PRECACHE_SOUND("controller/sunofgod_fire.wav");
...
int CController::IgnoreConditions ( void )
{
...
void CController :: RunTask ( Task_t *pTask )
{
	...
	if (m_flShootEnd > gpGlobals->time)
	{
		...
		while (m_flShootTime < m_flShootEnd && m_flShootTime < gpGlobals->time)
		{
			...
			if (m_hEnemy != NULL)
			{
				...
				if(flDist <= 512){
				vecDir = Intersect( vecSrc, m_hEnemy->BodyTarget( pev->origin ), m_vecEstVelocity, 1250 );
				}
				else if(flDist <= 1024){
				vecDir = Intersect( vecSrc, m_hEnemy->BodyTarget_c( pev->origin ), m_vecEstVelocity, 1250 );
				}
				else if(flDist <= 1536){
				vecDir = Intersect( vecSrc, m_hEnemy->BodyTarget_o( pev->origin ), m_vecEstVelocity, 1250 );
				}
				else{
				vecDir = Intersect( vecSrc, m_hEnemy->BodyTarget_z( pev->origin ), m_vecEstVelocity, 1250 );
				}
				...
				if ( FClassnameIs(pev, "monster_alien_controller_big")){
				pBall= (CBaseMonster*)Create( "controller_energy_ball2", vecSrc, pev->angles, edict() );
				}
				else{
	...
	case TASK_WAIT_PVS:
		...
		if (!m_fInCombat)
		{
			...
			{	//25%���ʻ���һ��
				if ( RANDOM_LONG(0,100) < 25 && m_iBall[0] > 128 && m_iBall[1] > 128 && HasConditions ( bits_COND_CAN_RANGE_ATTACK2 ) ){
...
void CController :: RunAI( void )
{
	...
	for (int i = 0; i < 2; i++)
	{
		if (m_pBall[i] == NULL)
		{
			...
			if ( FClassnameIs(pev, "monster_alien_controller_big")){
			m_pBall[i]->SetTransparency( kRenderTransAdd, 255, 32, 32, 255, kRenderFxNone );
...
void CControllerHeadBall :: HuntThink( void  )
{
	...
	if(m_hEnemy != NULL){
		...
		if ((m_hEnemy->Center() - pev->origin).Length() < dist)
		{
			TraceResult tr;
...
LINK_ENTITY_TO_CLASS( controller_energy_ball2, CControllerZapBall );
...
void CControllerZapBall :: Spawn( void )
{
	...
	MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
		...
		WRITE_BYTE( 3 ); // life
		WRITE_BYTE( 2 );  // width
		if ( FClassnameIs(pev, "controller_energy_ball2")){
		WRITE_BYTE( 255 );   // r, g, b
		WRITE_BYTE( 32 );   // r, g, b
		WRITE_BYTE( 32 );   // r, g, b
		pev->rendercolor.y = 32;
		pev->rendercolor.z = 32;
		pev->scale = 0.5;
		pev->dmg = 2;
		}
		else{
