...
//	}
	...
	m_killed_exp = 300;
	m_rpgms_level = 60;
	pev->netname = MAKE_STRING( "Apache" );
}
...
void CApache :: Killed( entvars_t *pevAttacker, int iGib )
{
	...
	{
		m_flNextRocket = gpGlobals->time + 8.0;
	}
}
...
void CApache :: DyingThink( void )
{
	...
	{
		CSoundEnt::InsertSound ( bits_SOUND_DANGER, pev->origin + pev->velocity * 0.5, 500, 0.2 );

		Vector origin_rad = pev->origin + Vector(RANDOM_FLOAT( -150, 150 ),RANDOM_FLOAT( -150, 150 ),RANDOM_FLOAT( -150, 50 ));
		FX_Explosion( origin_rad, EXPLOSION_WHL_SHARD );
	...
	else
	{
		...
		MESSAGE_END();
		*/
		FX_Explosion( vecSpot + Vector(0,0,256), EXPLOSION_C4 );
		...
		*/
		...
		RadiusDamage( pev->origin, pev, pev, 320, CLASS_NONE, DMG_BLAST );
...
void CApache :: HuntThink( void )
{
	...
	if ( !(pev->spawnflags & SF_MONSTER_PRISONER) && pev->frags == 0 ){
		if ((m_iRockets % 2) == 1)
		{
			FireRocket( );
...
BOOL CApache :: FireGun( )
{
	...
	{
#if 1
		FireBullets( 1, posGun, vecGun, Vector(0.04,0.04,0.04), 8192, BULLET_12MM, 1 , 625);
...
void CApache::TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	// ignore blades
	if ( (bitsDamageType & (DMG_ENERGYBEAM|DMG_BULLET|DMG_CLUB|DMG_SHOCK)) ){
		flDamage -= 60;//�Ƚ��ܵ������˺�����
	}
