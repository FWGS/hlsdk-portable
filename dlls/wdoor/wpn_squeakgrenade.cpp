...
{
	if ( m_hOwner != NULL && (m_hOwner->pev->flags & FL_CLIENT) )
	{
...
void CSqueakGrenade :: Spawn( void )
{
	...
	if ( pev->owner ){
		...
		if ( (m_hOwner->pev->flags & FL_CLIENT) ){
		pev->team = 1;
		}
	}
...
void CSqueakGrenade :: Killed( entvars_t *pevAttacker, int iGib )
{
	...
	if (m_hOwner != NULL){
		::RadiusDamage2(pev->origin, pev, m_hOwner->pev, 32, 64, cteam, DMG_BLOOD);
	}
	else{
		::RadiusDamage2(pev->origin, pev, pev, 32, 64, cteam, DMG_BLOOD);
	}
...
void CSqueakGrenade::SuperBounceTouch( CBaseEntity *pOther )
{
	...
	/*
	else if(pOther == m_hOwner){
		CBasePlayer *pPlayer = GetClassPtr((CBasePlayer *)pOther->pev);
		if(pPlayer){
			pPlayer->GiveAmmo( 1, "Snarks", SNARK_MAX_CARRY );
			SetTouch( NULL );
			UTIL_Remove( this );
			return;
		}
	...
	if ( pOther->pev->takedamage && m_flNextAttack < gpGlobals->time )
	{
		...
		if (tr.pHit == pOther->edict())
		{
			...
			if (tr.pHit->v.modelindex != pev->modelindex)
			{
				...
				ClearMultiDamage( );
				pOther->TraceAttack(pev, 12, gpGlobals->v_forward, &tr, DMG_BULLET ); 
...
LINK_ENTITY_TO_CLASS( weapon_snark_full, CSqueak );
...
void CSqueak::Spawn( )
{
	...
	#ifndef CLIENT_DLL
	if ( FClassnameIs( pev, "weapon_snark_full" ) ){
	m_iDefaultAmmo = 15;
	}
	#endif
