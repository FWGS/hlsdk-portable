...
#define GARG_DAMAGE					(DMG_SHOCK|DMG_ENERGYBEAM|DMG_CRUSH|DMG_MORTAR|DMG_BLAST|DMG_DARK|DMG_ENERGYBLAST|DMG_VALVE_SWORD)
...
public:
	...
	void Precache( void );
	void Think( void );
	static CStomp *StompCreate( entvars_t *pevOwner, const Vector &origin, const Vector &end, float speed );
...
LINK_ENTITY_TO_CLASS( gman_stomp, CStomp );

CStomp *CStomp::StompCreate( entvars_t *pevOwner, const Vector &origin, const Vector &end, float speed )
{
...
void CStomp::Spawn( void )
{
	...
	if ( FClassnameIs(pev, "gman_stomp")){
	pev->frags = 2;
	}
	else{
...
void CStomp::Think( void )
{
	...
	CSoundEnt::InsertSound ( bits_SOUND_DANGER, pev->origin, 600, 0.2 );

	if ( pev->frags == 2){
	dmg = 90;
	radiusd = 180;
	::RadiusDamage_limit( pev->origin, pev, VARS(pev->owner), dmg, radiusd, CLASS_HUMAN_ASS, DMG_BURN);
	}
	else{
	::RadiusDamage2( pev->origin, pev, VARS(pev->owner), dmg, radiusd, CLASS_ALIEN_MILITARY, DMG_SONIC);
	}
	...
	while ( gpGlobals->time - pev->dmgtime > STOMP_INTERVAL )
	{
		...
		for ( int i = 0; i < 2; i++ )
		{
			...
			if ( pSprite )
			{
				...
				if ( pev->frags == 2){
					pSprite->pev->scale = 1.25;
					pSprite->pev->velocity = Vector(RANDOM_FLOAT(-300,300),RANDOM_FLOAT(-300,300),175);
					pSprite->SetThink( &CStomp::SUB_Remove_fx );
					pSprite->pev->nextthink = gpGlobals->time + 0.4;

					if ( !FNullEnt( FIND_CLIENT_IN_PVS( edict() ) ) ){
					FX_Trail(pSprite->pev->origin, pSprite->entindex(), 145 );
					}
					...
					if(pev->frags == 1){
					pSprite->pev->scale = 1.25;
					pSprite->pev->velocity = Vector(RANDOM_FLOAT(-250,250),RANDOM_FLOAT(-250,250),175);
					}
					else{
...
public:
	...
	{
		if ( FClassnameIs(pev, "monster_gargantua_hell")){
		pev->absmin = pev->origin + Vector( -100, -100, 0 );
		pev->absmax = pev->origin + Vector( 100, 100, 267 );
		}
		else{
...
LINK_ENTITY_TO_CLASS( monster_gargantua_hell, CGargantua );
...
void CGargantua::StompAttack( void )
{
	...
	Vector vecEnd = (vecAim * 2048) + vecStart;
	...
	CStomp::StompCreate( pev,vecStart, trace.vecEndPos, speed );
...
void CGargantua :: FlameCreate( void )
{
	...
	{
		if ( i < 2 )
			m_pFlame[i] = CBeam::BeamCreate( GARG_BEAM_SPRITE_NAME, width );
		else
			m_pFlame[i] = CBeam::BeamCreate( GARG_BEAM_SPRITE2, width2 );
		if ( m_pFlame[i] )
		{
			int attach = i%2;
			...
			Vector vecEnd = (gpGlobals->v_forward * flame_len) + posGun;
...
void CGargantua :: FlameUpdate( void )
{
	...
	for ( i = 0; i < 2; i++ )
	{
		if ( m_pFlame[i] )
		{
			...
			Vector vecEnd = vecStart + (gpGlobals->v_forward * flame_len); //  - offset[i] * gpGlobals->v_right;
			...
			FlameDamage( vecStart, trace.vecEndPos, pev, pev, dmg, CLASS_NONE, DMG_BURN );
	...
	while ((pEntity = UTIL_FindEntityInSphere( pEntity, vecMid, searchRadius )) != NULL)
	{
		if ( pEntity->pev->takedamage != DAMAGE_NO )
		{
			...
			{// the explosion can 'see' this entity, so hurt them!
				...
				if (dist > rdst)
				{
					flAdjustedDamage = flDamage - (dist - rdst) * 0.4;
				...
				if ( pev->weapons == 114 && FClassnameIs(pEntity->pev, "func_door_breaker") ){
				//flAdjustedDamage = 3300.0;
				pEntity->pev->velocity.x *= 0.95;
				pEntity->pev->velocity.y *= 0.95;
				}
...
{
	if(pev->weapons == 111){
					CBaseEntity *pSpot = UTIL_FindEntityByTargetname( NULL, "breakable_train");
					if ( pSpot ){
						CBaseEntity *pTarget = Create( "npc_attack_flag", pSpot->Center() - Vector(0,0,128), Vector(0,0,0), NULL );
						pTarget->pev->frags = 1;
						pTarget->pev->health = 40;
						pTarget->pev->movetype = MOVETYPE_NOCLIP;
						pTarget->pev->velocity.y = 160;
						//pTarget->pev->effects = EF_BRIGHTFIELD;
					}
		pev->weapons = 114;
		m_seeTime = gpGlobals->time + 6;
		m_flameTime = gpGlobals->time;
	}
...
void CGargantua :: Spawn()
{
	...
	EyeOff();
	m_seeTime = gpGlobals->time + RANDOM_FLOAT(5,10);
	...
	pev->netname = MAKE_STRING( "Gargantua" );

	if ( FClassnameIs(pev, "monster_gargantua_hell")){
		SET_MODEL(ENT(pev), "models/garg_hell.mdl");
		...
		pev->netname = MAKE_STRING( "Gargantua.Hell" );
		m_ignoredamage		= 1;
	}
}
...
void CGargantua :: Precache()
{
	...
	PRECACHE_MODEL("models/garg_hell.mdl");
...
int CGargantua::TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if ( (bitsDamageType & DMG_SONIC) || (bitsDamageType & DMG_BURN) ){
		flDamage *= 0.2;
	}

	if ( (bitsDamageType & DMG_CLUB) || (bitsDamageType & DMG_ENERGYBEAM) 
	|| (bitsDamageType & DMG_SLASH) || (bitsDamageType & DMG_CRUSH)){
		flDamage *= 0.8;
	}
...
BOOL CGargantua :: FCanCheckAttacks ( void )
{
...
//	ALERT(at_aiconsole, "CheckMelee(%f, %f)\n", flDot, flDist);
	...
	if (flDot >= 0.7)
	{
		if ( FClassnameIs(pev, "monster_gargantua_hell") && flDist <= 120){
			return TRUE;
		}
		else if (flDist <= GARG_ATTACKDIST)
			return TRUE;
	}
	return FALSE;
}
...
//	ALERT(at_aiconsole, "CheckMelee(%f, %f)\n", flDot, flDist);
	...
	if ( gpGlobals->time > m_flameTime && m_cleardally_enemy > 0 )
	{
		if ( FClassnameIs(pev, "monster_gargantua_hell")){
			if (flDot >= 0.8 && flDist > 120)
			{
				if ( flDist <= 450 )
					return TRUE;
			}
...
BOOL CGargantua::CheckRangeAttack1( float flDot, float flDist )
{
	...
	{
		if (flDot >= 0.5 && flDist > GARG_ATTACKDIST)
		{
				return TRUE;
		}
	}
...
void CGargantua::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	case GARG_AE_SLASH_LEFT:
		{
			if(m_hEnemy != NULL){
				...
				if ( tr.flFraction < 1.0){
					if(( vecSrc - tr.vecEndPos).Length() <= dist){
					...
					{
					pEntity->pev->velocity = pEntity->pev->velocity + (pEntity->pev->origin - pev->origin).Normalize() * 300 - gpGlobals->v_right * 600;
					}
					ClearMultiDamage( );
					pEntity->TraceAttack(pev, dmg, gpGlobals->v_forward, &tr, DMG_CRUSH); 
	...
	case GARG_AE_BREATHE:
		EMIT_SOUND_DYN ( edict(), CHAN_VOICE, pAlertSounds[ RANDOM_LONG(0,ARRAYSIZE(pAlertSounds)-1) ], 1.0, ATTN_GARG, 0, PITCH_NORM + RANDOM_LONG(-10,10) );
		break;
	...
	case TASK_FLAME_SWEEP:
		...
		else
		{
			...
			if ( pEnemy )
			{
				...
				Vector dir = pEnemy->BodyTarget_o(org) - org;
