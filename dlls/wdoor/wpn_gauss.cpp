...
BOOL CGauss::Deploy( )
{
	...
	return DefaultDeploy( "models/v_gauss.mdl", 0, GAUSS_DRAW, "gauss" );
}
...
#ifndef CLIENT_DLL
			m_pPlayer->TakeDamage( VARS(eoNullEntity), VARS(eoNullEntity), 25, DMG_SHOCK );
...
void CGauss::StartFire( void )
{
	...
	{
		flDamage = 270 * (( gpGlobals->time - m_pPlayer->m_flStartCharge) / GetFullChargeTime() );
	}
	...
	if (m_fInAttack != 3)
	{
		...
		m_pPlayer->m_darkposion = 0;//隐身取消 Bug Fix 2.0
		}

		if ( !m_fPrimaryFire )
		{
			m_pPlayer->pev->velocity.x = m_pPlayer->pev->velocity.x - gpGlobals->v_forward.x * flDamage * 3;
			m_pPlayer->pev->velocity.y = m_pPlayer->pev->velocity.y - gpGlobals->v_forward.y * flDamage * 3;
			m_pPlayer->pev->velocity.z = m_pPlayer->pev->velocity.z - gpGlobals->v_forward.z * flDamage * 2;
		}
#endif
	...
	while (flDamage > 10 && nMaxHits > 0)
	{
		...
		FX_Explosion( tr.vecEndPos, EXPLOSION_BEAM_FLESHIMPACT);
			...
			if ( flDamage >= 90){
				...
				::RadiusDamage(tr.vecEndPos+(tr.vecPlaneNormal * 12), pev,m_pPlayer->pev, 15 + flDamage * 0.2, 100 + flDamage * 0.2, CLASS_NONE, DMG_ENERGYBEAM );
				}
			}
		...
		if (pEntity->pev->takedamage)
		{
			if(pEntity->pev != m_pPlayer->pev){
				...
				int tex = (int)TEXTURETYPE_Trace(&tr, vecOrigSrc, vecDest);
				int surface = (int)SURFACETYPE_Trace(&tr, vecOrigSrc, vecDest,CLASS_PLAYER,0);
				FX_ImpBullet( tr.vecEndPos, tr.vecPlaneNormal, vecOrigSrc, surface, BULLET_CROWBAR, (float)tex );
