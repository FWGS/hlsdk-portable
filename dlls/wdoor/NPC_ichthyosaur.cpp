...
{
	if ( flDot > -0.7 && (m_bOnAttack || ( flDist <= 256 && m_idealDist <= 256)))
	{
		return TRUE;
	}
	...
	case ICHTHYOSAUR_AE_SHAKE_LEFT:
		{
			if (m_hEnemy != NULL && FVisible( m_hEnemy ))
			{
				...
				if ( (m_hEnemy->BodyTarget_o( pev->origin ) - pev->origin).Length() > 96)
					break;

				Vector vecShootDir = ShootAtEnemy( pev->origin );
				UTIL_MakeAimVectors ( pev->angles );

				if (DotProduct( vecShootDir, gpGlobals->v_forward ) > 0.5)
				{
					m_bOnAttack = TRUE;
					pHurt->pev->punchangle.z += -18;
					pHurt->pev->punchangle.x += 5;
					pHurt->pev->velocity = pHurt->pev->velocity - gpGlobals->v_forward * 300;
					if (pHurt->IsPlayer())
					{
						pHurt->pev->angles.x += RANDOM_FLOAT( -35, 35 );
						pHurt->pev->angles.y += RANDOM_FLOAT( -70, 70 );
						pHurt->pev->angles.z = 0;
						pHurt->pev->fixangle = TRUE;
					}
					pHurt->TakeDamage( pev, pev, dmg, DMG_SLASH );
				}
			}
			BiteSound();
...
void CIchthyosaur :: Spawn()
{
	...
	m_killed_exp = 180;
	m_rpgms_level = 60;
	pev->netname = MAKE_STRING( "Ichthyosaur" );
}
...
void CIchthyosaur::MonsterThink ( void )
{
	...
	if (pev->deadflag == DEAD_NO)
	{
		if (m_MonsterState != MONSTERSTATE_SCRIPT)
		{
			...
			{
			m_flVelocityModifier += 0.1;
			pev->velocity = pev->velocity * m_flVelocityModifier;
			}
			else if (m_flVelocityModifier >= 1){
			Swim( );
			}
...
void CIchthyosaur :: RunAI( void )
{
	CBaseMonster :: RunAI();
	if(pev->health > 0){
		if(pev->waterlevel == 0 && pev->health > 0){
