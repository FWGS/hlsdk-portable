...
LINK_ENTITY_TO_CLASS( monster_houndeye_big, CHoundeye );
...
void CHoundeye :: RunAI( void )
{
	CBaseMonster :: RunAI();

	if (pev->sequence == LookupActivity ( ACT_RUN )){
		if ( FClassnameIs ( pev, "monster_houndeye_big" ) ){
		m_flGroundSpeed = 600;
		}
	}
}
...
void CHoundeye :: Spawn()
{
	...
	if ( FClassnameIs(pev, "monster_houndeye_big")){
		SET_MODEL(ENT(pev), "models/houndeye_giant.mdl");
		...
		m_killed_exp = 60;
		m_rpgms_level = 35;
		pev->netname = MAKE_STRING( "Houndeye.Hell" );
		...
		m_killed_exp = 20;
		m_rpgms_level = 20;
		pev->netname = MAKE_STRING( "Houndeye" );
...
void CHoundeye :: Precache()
{
	...
	PRECACHE_MODEL("models/houndeye_giant.mdl");
...
void CHoundeye :: SonicAttack ( void )
{
	...
	MESSAGE_BEGIN( MSG_PAS, SVC_TEMPENTITY, pev->origin );
		...
		WRITE_COORD( pev->origin.x);
		WRITE_COORD( pev->origin.y);
		WRITE_COORD( pev->origin.z + 16 + radius / .2); // reach damage radius over .3 seconds
	...
	MESSAGE_BEGIN( MSG_PAS, SVC_TEMPENTITY, pev->origin );
		...
		WRITE_COORD( pev->origin.x);
		WRITE_COORD( pev->origin.y);
		WRITE_COORD( pev->origin.z + 16 + ( radius / 2 ) / .2); // reach damage radius over .3 seconds
		WRITE_SHORT( m_iSpriteTexture );
		WRITE_BYTE( 0 ); // startframe
		WRITE_BYTE( 0 ); // framerate
		WRITE_BYTE( 2 ); // life
	...
	{
		// Ⱥ���˺���
		dmg += SquadCount() * 10 - 10;
	}
	*/

	::RadiusDamage2( pev->origin, pev, pev, dmg, radius_dmg, CLASS_NONE, DMG_SONIC);
}
