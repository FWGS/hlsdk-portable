...
LINK_ENTITY_TO_CLASS( monster_satchel_green, CSatchelCharge );
//=========================================================
...
void CSatchelCharge :: Spawn( void )
{
	...
	if ( FClassnameIs( pev, "monster_satchel_green" ) ){
	SET_MODEL(ENT(pev), "models/w_satchel_green.mdl");
	pev->health = 150;
	}
	else{
...
void CSatchelCharge :: SatchelThink( void )
{
	...
	pev->nextthink = gpGlobals->time + 0.1;
		...
		if ( FClassnameIs( pev, "monster_satchel_green" ) ){
			...
			if(pev->waterlevel >= 2 || pevOwner->deadflag != DEAD_NO || pev->health <= 0){
			UTIL_Remove( this );
			return;
			}
			else{
...
void CSatchel::Precache( void )
{
	...
	PRECACHE_MODEL("models/w_satchel_green.mdl");
...
BOOL CSatchel::Deploy( )
{
	...
	if ( m_chargeReady )
		return DefaultDeploy( "models/v_satchel_radio.mdl", 0, SATCHEL_RADIO_DRAW, "trip" );
	else
		return DefaultDeploy( "models/v_satchel.mdl", 0, SATCHEL_DRAW, "trip" );
...
void CSatchel::Throw( void )
{
	if ( m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] )
	{
		...
		Vector vecThrow = gpGlobals->v_forward * 300 + m_pPlayer->pev->velocity;
