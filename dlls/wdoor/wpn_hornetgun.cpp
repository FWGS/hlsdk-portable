...
void CHgun::PrimaryAttack()
{
	...
	pHornet->pev->velocity = gpGlobals->v_forward * 400;

	Vector vecSrc;
	UTIL_MakeVectors( m_pPlayer->pev->v_angle );
	vecSrc = m_pPlayer->GetGunPosition( ) + gpGlobals->v_forward * 16 + gpGlobals->v_right * 12 + gpGlobals->v_up * -12;
...
void CHgun::SecondaryAttack( void )
{
	...
	vecSrc = m_pPlayer->GetGunPosition( ) + gpGlobals->v_forward * 16 + gpGlobals->v_right * RANDOM_LONG(-4,4) + gpGlobals->v_up * RANDOM_LONG(-4,4);

	pHornet = CBaseEntity::Create( "hornet", vecSrc, m_pPlayer->pev->v_angle, m_pPlayer->edict() );
	pHornet->pev->velocity = gpGlobals->v_forward * 1600;
	pHornet->pev->health = 10;
