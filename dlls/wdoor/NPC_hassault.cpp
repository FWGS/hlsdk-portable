...
{
	DEFINE_FIELD( CHAssault, m_fGunDrawn, FIELD_BOOLEAN ),
	DEFINE_FIELD( CHAssault, m_flNextGrenadeCheck, FIELD_TIME ),
	DEFINE_FIELD( CHAssault, m_flNextPainTime, FIELD_TIME ),
//	DEFINE_FIELD( CHAssault, m_flLastEnemySightTime, FIELD_TIME ), // don't save, go to zero
	DEFINE_FIELD( CHAssault, m_vecTossVelocity, FIELD_VECTOR ),
	DEFINE_FIELD( CHAssault, m_fThrowGrenade, FIELD_BOOLEAN ),
	DEFINE_FIELD( CHAssault, m_fStanding, FIELD_BOOLEAN ),
	DEFINE_FIELD( CHAssault, m_fFirstEncounter, FIELD_BOOLEAN ),
...
// started moving.
//=========================================================
void CHAssault :: SpeakSentence( void )
{
...
void CHAssault::Killed( entvars_t *pevAttacker, int iGib )
{
	...
	if ( pev->weapons >= 1 && GetBodygroup( GUN_GROUP ) != 1 && g_gibexp_max < 120 && !m_undropgun)
	{// throw a gun if the grunt has one
		SetBodygroup( GUN_GROUP, 1 );
		CBaseEntity *pGun;


		GetAttachment( 1, vecGunPos, vecGunAngles );
		pGun = DropItem( "weapon_minigun_drop", vecGunPos, vecGunAngles );
...
BOOL CHAssault :: CheckRangeAttack1 ( float flDot, float flDist )
{
	...
	if(m_hEnemy != NULL){
			...
			else if(m_aimenemy_mod != 7 && flDist < (dist * 0.5) ){//�о���
			m_aimenemy_mod = 7;
			}
...
void CHAssault :: Shoot ( void )
{
	...
	vecShootOrigin = pev->origin + Vector(0,0,40);
	...
	FireBullets(1, vecShootOrigin, vecShootDir, Vector(0.09,0.09,0.09), 2048, iBulletType,1); // shoot +-5 degrees
...
void CHAssault :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		...
		{
				Shoot();
				EMIT_SOUND_DYN( ENT(pev), CHAN_WEAPON, "weapons/minigun_fire.wav", 1, 0.7, 0, 100 );
		...
		case HGRUNT_AE_KICK:
		{
			...
			EMIT_SOUND_DYN ( ENT(pev), CHAN_WEAPON, "newadd/fist_hitbod2.wav", 1.0, ATTN_NORM, 0, 100 + RANDOM_LONG(-5,5) );
				...
				CBaseEntity *pHurt = CheckTraceHullAttack( 80, 20, DMG_SLASH );
				if ( pHurt )
				{
...
void CHAssault :: Spawn()
{
	...
	pev->netname = MAKE_STRING( "HECU.Assault" );
}
...
Schedule_t	slAssaultVictoryDance[] =
{
	{ 
		tlAssaultVictoryDance,
		ARRAYSIZE ( tlAssaultVictoryDance ), 
		bits_COND_NEW_ENEMY		|
		bits_COND_SEE_ENEMY		|
...
Task_t tlAssaultEstablishLineOfFire_strap[] = 
{
	{ TASK_SET_FAIL_SCHEDULE,	(float)SCHED_GRUNT_ELOF_FAIL	},
	{ TASK_GET_PATH_TO_ENEMY,	(float)0						},
...
Schedule_t slAssaultEstablishLineOfFire_strap[] =
{
	{ 
		tlAssaultEstablishLineOfFire_strap,
		ARRAYSIZE ( tlAssaultEstablishLineOfFire_strap ),
		bits_COND_NEW_ENEMY			|
		bits_COND_ENEMY_DEAD		|
		bits_COND_CAN_RANGE_ATTACK1	|
		bits_COND_CAN_MELEE_ATTACK1	|
...
//=========================================================
Task_t	tlAssaultSignalSuppress[] =
{
	{ TASK_STOP_MOVING,					0						},
...
Schedule_t	slAssaultSignalSuppress[] =
{
	{ 
		tlAssaultSignalSuppress,
		ARRAYSIZE ( tlAssaultSignalSuppress ), 
		bits_COND_ENEMY_DEAD		|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE		|
		bits_COND_HEAR_SOUND		|
...
Schedule_t	slAssaultTakeCover[] =
{
	{ 
		tlAssaultTakeCover1,
		ARRAYSIZE ( tlAssaultTakeCover1 ), 
		0,
		0,
		"TakeCover"
	},
...
Schedule_t	slAssaultGrenadeCover[] =
{
	{ 
		tlAssaultGrenadeCover1,
		ARRAYSIZE ( tlAssaultGrenadeCover1 ), 
		0,
		0,
		"TakeCover"
	},
...
// drop grenade then run to cover.
//=========================================================
Task_t	tlAssaultTossGrenadeCover1[] =
{
	{ TASK_STOP_MOVING,						(float)0							},
	{ TASK_FACE_ENEMY,						(float)0							},
...
Schedule_t	slAssaultTossGrenadeCover[] =
{
	{ 
		tlAssaultTossGrenadeCover1,
		ARRAYSIZE ( tlAssaultTossGrenadeCover1 ), 
		0,
		0,
		"TossGrenadeCover"
	},
...
Task_t	tlAssaultTakeCoverFromBestSound2[] =
{
...
Schedule_t	slAssaultTakeCoverFromBestSound2[] =
{
	{ 
		tlAssaultTakeCoverFromBestSound2,
		ARRAYSIZE ( tlAssaultTakeCoverFromBestSound2 ), 
		0,
		0,
...
Schedule_t	slAssaultRangeAttack1A[] =
{
	{ 
		tlAssaultRangeAttack1A,
		ARRAYSIZE ( tlAssaultRangeAttack1A ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_ENEMY_DEAD		|
		bits_COND_HEAVY_DAMAGE		|
...
Schedule_t	slAssaultRangeAttack1B[] =
{
	{ 
		tlAssaultRangeAttack1B,
		ARRAYSIZE ( tlAssaultRangeAttack1B ), 
		bits_COND_NEW_ENEMY			|
		bits_COND_ENEMY_DEAD		|
		bits_COND_HEAVY_DAMAGE		|
		bits_COND_ENEMY_OCCLUDED	|
...
Schedule_t	slAssaultRepel[] =
{
	{ 
		tlAssaultRepel,
		ARRAYSIZE ( tlAssaultRepel ), 
		bits_COND_SEE_ENEMY			|
		bits_COND_NEW_ENEMY			|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE		|
...
Schedule_t	slAssaultRepelAttack[] =
{
	{ 
		tlAssaultRepelAttack,
		ARRAYSIZE ( tlAssaultRepelAttack ), 
		bits_COND_ENEMY_OCCLUDED,
		0,
		"Repel Attack"
	},
...
Schedule_t	slAssaultRepelLand[] =
{
	{ 
		tlAssaultRepelLand,
		ARRAYSIZE ( tlAssaultRepelLand ), 
		bits_COND_SEE_ENEMY			|
		bits_COND_NEW_ENEMY			|
		bits_COND_LIGHT_DAMAGE		|
		bits_COND_HEAVY_DAMAGE		|
...
DEFINE_CUSTOM_SCHEDULES( CHAssault )
{
	slAssaultFail,
	slAssaultCombatFail,
	slAssaultVictoryDance,
	slAssaultEstablishLineOfFire,
	slAssaultFoundEnemy,
	slAssaultCombatFace,
	slAssaultSignalSuppress,
	slAssaultSuppress,
	slAssaultWaitInCover,
	slAssaultTakeCover,
	slAssaultGrenadeCover,
	slAssaultTossGrenadeCover,
	slAssaultTakeCoverFromBestSound,
	slAssaultTakeCoverFromBestSound2,
	slAssaultHideReload,
	slAssaultSweep,
	slAssaultRangeAttack1A,
	slAssaultRangeAttack1B,
	slAssaultRangeAttack2,
	slAssaultRepel,
	slAssaultRepelAttack,
	slAssaultRepelLand,
	slAssaultEstablishLineOfFire_strap,
};
...
Schedule_t *CHAssault :: GetSchedule( void )
{
	...
	case MONSTERSTATE_ALERT:
		{
			/*
			if ( HasConditions ( bits_COND_HEAR_SOUND ) && m_allydeadcheck == 0)
			{
					CSound *pSound;
					pSound = PBestSound();
...
// damaged just a little
			else if ( HasConditions( bits_COND_HEAVY_DAMAGE ) )
			{
					...
					if ( iPercent <= 90 && m_hEnemy != NULL && pev->health <= pev->max_health * 0.4 )
					//���ˣ�90%��������
					{
						return GetScheduleOfType( SCHED_TAKE_COVER_FROM_ENEMY );
					}
					else if ( iPercent <= 10 && m_hEnemy != NULL )
					//������10%��������
					{
						return GetScheduleOfType( SCHED_TAKE_COVER_FROM_ENEMY );
					}
...
Schedule_t* CHAssault :: GetScheduleOfType ( int Type ) 
{
	...
	case SCHED_RANGE_ATTACK1:
		{
			return &slAssaultRangeAttack1A[ 0 ];
		}
	...
	case SCHED_VICTORY_DANCE:
		{
			return &slAssaultVictoryDance[ 0 ];
		}
	...
	case SCHED_GRUNT_REPEL:
		{
			if (pev->velocity.z > -128)
				pev->velocity.z -= 32;
			return &slAssaultRepel[ 0 ];
		}
	case SCHED_GRUNT_REPEL_ATTACK:
		{
			if (pev->velocity.z > -128)
				pev->velocity.z -= 32;
			return &slAssaultRepelAttack[ 0 ];
		}
	case SCHED_GRUNT_REPEL_LAND:
		{
			return &slAssaultRepelLand[ 0 ];
		}
	default:
		{
...
//=========================================================
// CHAssaultRepel - when triggered, spawns a monster_human_grunt
// repelling down a line.
//=========================================================

class CHAssaultRepel : public CBaseMonster
{
public:
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( monster_assault_repel, CHAssaultRepel );

void CHAssaultRepel::Spawn( void )
{
	Precache( );
	pev->solid = SOLID_NOT;

	SetUse( &CHAssaultRepel::RepelUse );
}

void CHAssaultRepel::Precache( void )
{
	UTIL_PrecacheOther( "monster_human_assault" );
	m_iSpriteTexture = PRECACHE_MODEL( "sprites/rope.spr" );
}

void CHAssaultRepel::RepelUse ( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	TraceResult tr;
	...
	*/
	...
	CBaseEntity *pEntity = Create( "monster_human_assault", pev->origin, pev->angles );
	...
	pBeam->SetThink( &CHAssaultRepel::SUB_Remove );
...
{
	PRECACHE_MODEL("models/hgrunt_f.mdl");
	SET_MODEL(ENT(pev), "models/hgrunt_f.mdl");
