...
void CHAssassin :: Shoot ( void )
{
	...
	if(pev->frags == 1){
	FireBullets(1, vecShootOrigin, vecShootDir, Vector( 0, 0, 0 ), 4096, BULLET_338Magnum ); // shoot +-8 degrees
	pev->frags = 0;
	}
	else{
	FireBullets(1, vecShootOrigin, vecShootDir, Vector( 0.015, 0.015, 0.015 ), 2048, BULLET_57mm ); // shoot +-8 degrees
	}
	...
	case ASSASSIN_AE_TOSS1:
		{
			...
			m_flNextGrenadeCheck = gpGlobals->time + 5;// wait six seconds before even looking again to see if a grenade can be thrown.
...
BOOL CHAssassin :: CheckRangeAttack1 ( float flDot, float flDist )
{
	if ( !HasConditions( bits_COND_ENEMY_OCCLUDED ) && flDist > 64 && flDist <= 2048 /* && flDot >= 0.5 */ /* && NoFriendlyFire() */ )
	{
		...
		UTIL_TraceLine( vecSrc, m_hEnemy->BodyTarget_o(vecSrc), dont_ignore_monsters, ENT(pev), &tr);
...
BOOL CHAssassin :: CheckRangeAttack2 ( float flDot, float flDist )
{
	...
	if ( m_flNextGrenadeCheck < gpGlobals->time && flDist <= 800 && flDist >= 200 )
	{
