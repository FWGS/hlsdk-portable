...
void FindHullIntersection6( const Vector &vecSrc, TraceResult &tr, float *mins, float *maxs, edict_t *pEntity )
{
	int			i, j, k;
	float		distance;
...
int CHammer::Swing( int fFirst )
{
	...
	if ( tr.flFraction >= 1.0 )
	{
		...
		if ( tr.flFraction < 1.0 )
		{
			...
			if ( !pHit || pHit->IsBSPModel() )
				FindHullIntersection6( vecSrc, tr, VEC_DUCK_HULL_MIN, VEC_DUCK_HULL_MAX, m_pPlayer->edict() );
	...
	else
	{
		...
		{
		case 0:
			SendWeaponAnim( HAMMER_ATTACK1HIT ); break;
		case 1:
			SendWeaponAnim( HAMMER_ATTACK2HIT ); break;
		case 2:
			SendWeaponAnim( HAMMER_ATTACK3HIT ); break;
		}
		...
		pEntity->TraceAttack(m_pPlayer->pev, 7, gpGlobals->v_forward, &tr, DMG_CLUB ); 
