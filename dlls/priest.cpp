...
#define SF_AUTO_FIREONCE		0x0001      // env_warpball's spawnflag
...
LINK_ENTITY_TO_CLASS( monster_alien_priest, CPriest );

TYPEDESCRIPTION	CPriest::m_SaveData[] =
{
	DEFINE_ARRAY( CPriest, m_pBall, FIELD_CLASSPTR, 2 ),
	DEFINE_ARRAY( CPriest, m_iBall, FIELD_INTEGER, 2 ),
	DEFINE_ARRAY( CPriest, m_iBallTime, FIELD_TIME, 2 ),
	DEFINE_ARRAY( CPriest, m_iBallCurrent, FIELD_INTEGER, 2 ),
	DEFINE_FIELD( CPriest, m_vecEstVelocity, FIELD_VECTOR ),
};
IMPLEMENT_SAVERESTORE( CPriest, CSquadMonster );


const char *CPriest::pAttackSounds[] = 
{
...
const char *CPriest::pIdleSounds[] = 
{
...
const char *CPriest::pAlertSounds[] = 
{
...
void CPriest :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	switch( pEvent->event )
	{
		...
		case CONTROLLER_AE_BALL_SHOOT:
		{
			...
			CBaseMonster *pBall = (CBaseMonster*)Create( "priest_head_ball", vecStart, pev->angles, edict() );
...
void CPriest :: Precache()
{
	...
	UTIL_PrecacheOther( "priest_energy_ball" );
	UTIL_PrecacheOther( "priest_head_ball" );
...
Schedule_t slPriestChaseEnemy[] =
{
	{ 
		tlPriestChaseEnemy,
		ARRAYSIZE ( tlPriestChaseEnemy ),
		bits_COND_NEW_ENEMY			|
...
Schedule_t	slPriestStrafe[] =
{
	{ 
		tlPriestStrafe,
		ARRAYSIZE ( tlPriestStrafe ),
		bits_COND_NEW_ENEMY,
		0,
		"ControllerStrafe"
	},
...
Schedule_t	slPriestTakeCover[] =
{
	{ 
		tlPriestTakeCover,
		ARRAYSIZE ( tlPriestTakeCover ), 
		bits_COND_NEW_ENEMY,
		0,
		"ControllerTakeCover"
	},
...
Schedule_t	slPriestFail[] =
{
	{
		tlPriestFail,
		ARRAYSIZE ( tlPriestFail ),
		0,
		0,
		"ControllerFail"
	},
...
// RunTask 
//=========================================================
void CPriest :: RunTask ( Task_t *pTask )
{
	...
	if (m_flShootEnd > gpGlobals->time)
	{
		...
		while (m_flShootTime < m_flShootEnd && m_flShootTime < gpGlobals->time)
		{
			...
			if (m_hEnemy != NULL)
			{
				...
				CBaseMonster *pBall = (CBaseMonster*)Create( "priest_energy_ball", vecSrc, pev->angles, edict() );
...
Schedule_t* CPriest :: GetScheduleOfType ( int Type ) 
{
	...
	case SCHED_CHASE_ENEMY:
		return slPriestChaseEnemy;
	case SCHED_RANGE_ATTACK1:
		return slPriestStrafe;
	case SCHED_RANGE_ATTACK2:
	case SCHED_MELEE_ATTACK1:
	case SCHED_MELEE_ATTACK2:
	case SCHED_TAKE_COVER_FROM_ENEMY:
		return slPriestTakeCover;
	case SCHED_FAIL:
		return slPriestFail;
	}
...
void CPriest :: Move ( float flInterval ) 
{
...
BOOL CPriest:: ShouldAdvanceRoute( float flWaypointDist )
{
...
int CPriest :: CheckLocalMove ( const Vector &vecStart, const Vector &vecEnd, CBaseEntity *pTarget, float *pflDist )
{
	TraceResult tr;
...
void CPriest::MoveExecute( CBaseEntity *pTargetEnt, const Vector &vecDir, float flInterval )
{
...
//=========================================================
class CPriestHeadBall : public CBaseMonster
{
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( priest_head_ball, CPriestHeadBall );



void CPriestHeadBall :: Spawn( void )
{
	Precache( );
	// motor
	pev->movetype = MOVETYPE_FLY;
...
void CPriestHeadBall :: Precache( void )
{
	PRECACHE_MODEL("sprites/flare6.spr");
...
void CPriestHeadBall :: HuntThink( void  )
{
	...
	if ((m_hEnemy->Center() - pev->origin).Length() < 64)
	{
		...
		CBaseEntity *pBall = (CBaseEntity*)Create( "env_warpball", tr.vecEndPos, pev->angles, edict() );
		
		pBall->pev->velocity = Vector( 0, 0, 32 );
		pBall->pev->spawnflags = SF_AUTO_FIREONCE;
		pBall->Use( this, this, USE_TOGGLE, 0);
		/*
		MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
		...
		MESSAGE_END();
        */
		UTIL_EmitAmbientSound( ENT(pev), tr.vecEndPos, "debris/beamstart9.wav", 0.5, ATTN_NORM, 0, RANDOM_LONG( 140, 160 ) );
...
void CPriestHeadBall :: DieThink( void  )
{
	UTIL_Remove( this );
}


void CPriestHeadBall :: MovetoTarget( Vector vecTarget )
{
	// accelerate
...
void CPriestHeadBall :: Crawl( void  )
{
	...
	CBaseEntity *pBall = (CBaseEntity*)Create( "env_warpball", vecPnt, pev->angles, edict() );
...
void CPriestHeadBall::BounceTouch( CBaseEntity *pOther )
{
...
class CPriestZapBall : public CBaseMonster
{
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( priest_energy_ball, CPriestZapBall );


void CPriestZapBall :: Spawn( void )
{
	Precache( );
	// motor
	pev->movetype = MOVETYPE_FLY;
...
void CPriestZapBall :: Precache( void )
{
	PRECACHE_MODEL("sprites/flare6.spr");
	// PRECACHE_SOUND("debris/zap4.wav");
...
void CPriestZapBall :: AnimateThink( void  )
{
	pev->nextthink = gpGlobals->time + 0.1;
...
void CPriestZapBall::ExplodeTouch( CBaseEntity *pOther )
{
	if (pOther->pev->takedamage)
	{
		...
		UTIL_EmitAmbientSound( ENT(pev), tr.vecEndPos, "debris/beamstart9.wav", 0.3, ATTN_NORM, 0, RANDOM_LONG( 90, 99 ) );
